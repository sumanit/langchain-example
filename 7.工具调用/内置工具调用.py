...
if __name__ == "__main__":
    ...
    # 绑定工具
    ollama_llm_with_tools = ollama_llm.bind_tools([file_tool])

    # 使用正确的消息格式
    messages = [HumanMessage(content="我是mac系统 请帮我在桌面找下doc文件?")]
    ...
    if hasattr(response, 'tool_calls') and response.tool_calls:
        ...
        print(f"触发工具调用: {tool_call['name']}")
        # 执行工具并更新消息历史
        args = tool_call['args']
        del args['dir_path']
        tool_result = file_tool.invoke(args)
        print(tool_result)
        messages.append({
            "role": "tool",
